...
The Data Collection Service uses a unified JSON structure, but specific ID fields change depending on the disaster source. Here is exactly what the POST JSON bodies look like when they hit the Main Backend (`http://localhost:8000/alerts/incoming`). These examples must guide your database schema and Pydantic validation.
